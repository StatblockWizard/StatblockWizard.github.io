    ...
    Content.forEach(element => {
        switch (element.type) {
            ...
            case 'string':
                ...
                break;
            case 'ability5e':
                AddToStatblockHtml(Oability5e(element))
                break;
            case 'skills5e':
            ...
            if (element.values) {
                element.values.forEach(value => {
                    ...
                    switch (value.type) {
                        ...
                        case "attack5e":
                            AddToStatblockHtml(Oattack5e(value, csselement));
                            result = true;
                            break;
                        case "spells5e":
...
function Oattack5e(value, csselement) {
    let c = value.caption;
    let at = ((value.attacktype) ? value.attacktype : value.weapontype); // for backward compatibility
    let a = value.attack
    let h = value.hit;
    if ((c + at + a + h) != '') {
        let l = P('', csselement.css);
        l.appendChild(CatchAttackNameInSpan(c, csselement.captioncss));
        l.appendChild(SPAN(attacktype5e()[at].text, csselement.attackcss));
...
function GetImgCSS() {
    ...
    .StatblockWizard-title {
        font-family: 'StatblockWizard Serif', Georgia, 'Times New Roman', Times, serif;
        font-size: 1.7em;
